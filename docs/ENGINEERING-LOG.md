# Engineering log: what changed and why
...
scraped. Weighted per category and re-weighted by query priorities: asking for a
_gaming_ phone raises the performance weight, _camera_ raises camera.
...
is how the ranking logic was iterated dozens of times against existing captured
runs, without a single new request.
...
## Round 2a: what the real Amazon payload taught us
...
## Round 2b: audio is a first-class category

Replaying the saved `sony wh-1000xm5` runs showed the pipeline ranking a ₹1,000
silicone case at #1 and the WH-1000XM5 itself last: the same failure mode as the
earphones bug, in a different category. Three fixes:
...
## Round 2c: naming a specific model
...
`eval-live/`: 96 tracked files of scratch debugging output. Gone too. The two
captures that mattered are preserved as proper fixtures:
...
- `doctor` absorbed `status` and `scrapers`, and gained `--query`, which prints
  the exact URL each platform would be sent, catching URL-builder bugs for free,
...
earbuds, laptops, TVs and accessories. It has to, in order to reject them and to
keep the funnel readable ("11 earbuds filtered out" is how the Reliance URL bug
was caught in the first place); only `phone` is rankable.
...
Note 13 Pro, POCO X6/X7 Pro, Galaxy A25/A35/A55, Pixel 8a and Nothing Phone
(2a), live in the ₹15k–45k bracket and simply do not appear in a "under 15000"
search. They should pay off on mid-range queries; that is untested, because
there is no captured payload for one.
...
1. **Enrichment was aimed at the wrong products.** The implementation sliced the
   ...
   was spent on ranks 1-14 (nearly all already in the KB) and never reached the
   unknown phones below them. The first measured run recovered _nothing_.
...
`BEST VALUE` moved to `Ai+ Pulse 2`: Unisoc T7250, 4.2★ from 9,500 buyers, which
is a defensible thing to tell someone to buy.
...
What _was_ built is the honest part: the detail cards now show the real checkout
number and the two things that genuinely vary:
...
4.1–4.3. The signal is in the review text, and it is reachable: the PDP has none
(JS-rendered) but `/product-reviews/` returns it in plain HTML:
...
3. **Anchors**: orderings that are uncontroversial on the captured fixture, plus
   adversarial products that must _not_ win.
...
largely imputed that ratio is an assertion rather than a measurement, yet it was
being scored at full weight. The value percentile is now scaled by how much of
the spec sheet is real, so a product cannot claim good value on specs nobody has
verified.
...
That Nokia's "20,000 mAh" and "50MP OIS" came from a _Similar products_
carousel, , the same contamination class as the Apple A17 incident, which is now
three separate occurrences. Beyond the earlier anchoring fixes, values are
